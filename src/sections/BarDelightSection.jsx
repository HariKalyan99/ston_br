...
import { barDelightsLists } from "../constants";
import { useMediaQuery } from "react-responsive";
import { useGSAP } from "@gsap/react";
import { SplitText } from "gsap/all";
import gsap from "gsap";

const BarDelightSection = () => {
  const isMobile = useMediaQuery({
    query: "(max-width: 768px)",
  });

  const [lists, setLists] = useState(barDelightsLists);

  useEffect(() => {
    if (isMobile) {
      setLists(barDelightsLists.slice(0, 3));
    } else {
      setLists(barDelightsLists);
    }
  }, [isMobile]);
          ...
          <div className="general-title relative flex flex-col justify-center items-center gap-24">
            ...
            >
              <div className="bg-yellow-brown pb-5 md:pt-0 pt-3 md:px-5 px-3 inline-block">
                <h2 className="text-milk-yellow">Menu strip</h2>
              </div>
            </div>
          </div>
        </div>
...
export default BarDelightSection;
